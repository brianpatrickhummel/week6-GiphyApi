...
function loadButtons() {
  ...
  for (var i = 0; i < animals.length; i++) {
    var button = $("<button>")
      .addClass("btn btn-primary animal")
      .attr({ "data-name": animals[i], type: "button" })
      .html("<p class=animalText>" + animals[i] + "</p>");
...
$(document).on("click", ".animal", function() {
  ...
  $.ajax({
    ...
    for (var j = 0; j < response.data.length; j++) {
      ...
      var gifsToAdd = $("<img>")
        .attr({ src: stillSource, "data-still": stillSource, "data-animate": animatedSource, "data-state": "still" })
        .addClass("gifs");
